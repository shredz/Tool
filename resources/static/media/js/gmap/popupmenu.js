...
var jquerypopupmenu = {
    ...
    showhidedelay: [500, 500], //delay before menu appears and disappears when mouse rolls over it, in milliseconds
    ...
    positionul: function($, $ul, e)
    {
        ...
        var docrightedge = $(document).scrollLeft() + $(window).width() //40 is to account for shadows in FF
        var docbottomedge = $(document).scrollTop() + $(window).height()
        if (istoplevel)
        { //if main popup menu DIV
            //            var x = e.pageX + this.popupmenuoffsets[0] //x pos of main popup menu UL
            //            var y = e.pageY + this.popupmenuoffsets[1] - 25
            //            //               		    var x = 160 
            //            //		    var y = 20
            ...
            //            var x = $ul.data('dimensions').w;
            //            var y = $ul.data('dimensions').h;

            //                        var objSourceElement = e.srcElement;
            var objSourceElement = e.target;

            var objHeight = objSourceElement.clientHeight;
            var objWidth = objSourceElement.clientWidth;

            var x = findObjectPosX(objSourceElement) + objWidth;
            var y = findObjectPosY(objSourceElement);
        }
        else
        { //if sub level popup menu UL
            var $parentli = $ul.data('$parentliref')
            var parentlioffset = $parentli.offset()
            var x = $ul.data('dimensions').parentliw + 00 //x pos of sub UL
            var y = 0
    ...
    buildpopupmenu: function($, $menu, $target)
    {
        ...
        $lis.each(function(i)
        {
            ...
            //$li.children("li:eq(0)").append( //add arrow images
            //	'<img src="' + jquerypopupmenu.arrowpath + '" class="rightarrowclass" style="border:0;" />'
            //)
